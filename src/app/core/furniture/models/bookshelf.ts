...
} from './bookshelf.constants'
import type { RowStyle } from './bookshelf.constants'

export type { RowStyle } from './bookshelf.constants'
...
export class BookShelf {
  ...
  private rowStyle: RowStyle = 'pattern'
  ...
  private buildAdditionalStorageSection(
    ...
    group.add(
      new Blank(thickness, yBase, 0, width - thickness, yBase + thickness, depth, origin, material.clone(), idRef.id++).build()
    )
    group.add(
      new Blank(
        thickness,
    ...
    group.add(
      new Blank(thickness, yBase, 0, thickness * 2, yBase + sectionHeight, depth, origin, material.clone(), idRef.id++).build()
    )
    group.add(
      new Blank(
        width - thickness * 2,
    ...
    for (let c = 1; c < columns; c++) {
      ...
      group.add(
        new Blank(x - thickness, yLow1, EPS, x, yHigh1, depth, origin, material.clone(), idRef.id++).build()
      )
    }
    for (let c = 1; c < columns; c++) {
      const x = thickness * 2 + c * (baseCellWidth + thickness)
      group.add(
        new Blank(x - thickness, yLow2, EPS, x, yHigh2, depth, origin, material.clone(), idRef.id++).build()
      )
    }
    if (withBack) {
      group.add(
  ...
  private rebuild() {
    ...
    const bottomSectionHeight =
      this.bottomStorageConfig !== null
        ? BookShelf.ADDITIONAL_STORAGE_PARTS * ROW_HEIGHT_MAP[this.bottomStorageConfig.height] + 3 * thickness
        : 0
    const topSectionHeight =
      this.topStorageConfig !== null
        ? BookShelf.ADDITIONAL_STORAGE_PARTS * ROW_HEIGHT_MAP[this.topStorageConfig.height] + 3 * thickness
        : 0
    ...
    for (let r = 0; r < rows; r++) {
      ...
      if (this.rowStyle === 'pixel') {
        const step = baseCellHeight * 0.5
        rowGroup.position.x = Math.round(rowGroup.position.x / step) * step
      }

      if (this.rowStyle === 'mosaic') {
        const availableWidth = openingWidth - (columns - 1) * thickness
        dividerPositions = []
        let x = thickness * 2
        const cellWidths: number[] = []
      ...
      if (this.rowStyle === 'pattern') {
        if (r % 3 === 0) rowGroup.scale.x = 1.1
        if (r % 3 === 1) rowGroup.position.x -= baseCellWidth * 0.2
        if (r % 3 === 2) rowGroup.position.x += baseCellWidth * 0.2
      }

      if (dividerPositions.length > 0) {
        const minX = thickness * 3
        // Keep last cell at least baseCellWidth so the rightmost door/drawer fits
      ...
      if (!slantNoLeftBlank) {
        rowGroup.add(
          new Blank(thickness, yBottom, 0, thickness * 2, yTop, depth, origin, material.clone(), idRef.id++).build()
        )
      }
  ...
  getTotalHeight(): number {
    const bottom =
      this.bottomStorageConfig !== null
        ? BookShelf.ADDITIONAL_STORAGE_PARTS * ROW_HEIGHT_MAP[this.bottomStorageConfig.height] +
          3 * this.thickness
        : 0
    const top =
      this.topStorageConfig !== null
        ? BookShelf.ADDITIONAL_STORAGE_PARTS * ROW_HEIGHT_MAP[this.topStorageConfig.height] +
          3 * this.thickness
        : 0
  ...
  getConfigJson(): BookShelfConfigJson {
    this.ensureRowConfigs()
    return {
