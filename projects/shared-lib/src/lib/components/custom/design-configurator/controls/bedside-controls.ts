    ...
    <div *transloco="let t">
      @if (store.activeMainTab() === 'model') {
        ...
        <div [class.hidden]="!['width','height','depth','density'].includes(store.activeControlTab())"
          ...
          <h5 class="font-label text-foreground/60 hidden lg:block">{{ t('sharedLib.designConfigurator.controls.dimensionsAndDensity') }}</h5>
          ...
          <div [class.hidden]="store.activeControlTab() !== 'density'" class="lg:block">
            <z-form-field>
