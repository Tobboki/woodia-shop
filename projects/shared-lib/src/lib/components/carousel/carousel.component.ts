...
export class ZardCarouselComponent {
  ...
  /** When set, carousel will scroll to this index on init; parent can also call scrollToIndex() to sync. */
  readonly zSelectedIndex = input<number>(0);
  ...
  onEmblaChange(type: EmblaEventType, emblaApi: EmblaCarouselType): void {
    if (type === 'init' || type === 'reInit') {
      ...
      if (type === 'init') {
        emblaApi.scrollTo(this.zSelectedIndex());
        this.#index = this.zSelectedIndex();
        this.selectedIndex.set(this.zSelectedIndex());
        this.checkNavigation(emblaApi);
  ...
  /** Public API: scroll to slide index (e.g. to sync with external state). */
